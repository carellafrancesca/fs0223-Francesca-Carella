...
let whoIsBigger2 = whoIsBigger(9,17);
...
function whatDayIsIt(){
  ...
  let today = new Date();
  let weekDay = daysOfTheWeek[today.getDay()];
  return weekDay
}
...
function deleteProp(oggettoDue, prop) {
  delete oggettoDue[prop];
  return oggettoDue;
}
console.log(deleteProp({ name: "Francesca", surname: "Carella", age: 23 }, "surname"));
...
function twentyOne (){
  let element2 = document.querySelectorAll("td");
}
...
function colore(){
  let elementoColore = document.querySelectorAll("a");
  for (i = 0; i < elementoColore.length; i++){
    elementoColore[i].style.backgroundColor = "red";
  }
}
colore();
...
function lista(){
  ...
  let newElenco = document.createElement("li");
  newElenco.innerHTML = "Nuovo elemento";
  elenco.appendChild(newElenco);
}  
